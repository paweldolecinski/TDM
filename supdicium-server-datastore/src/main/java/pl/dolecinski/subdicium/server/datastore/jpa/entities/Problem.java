 ...
 * the License.
 */
package pl.dolecinski.subdicium.server.datastore.jpa.entities;
...
import pl.dolecinski.subdicium.server.datastore.api.dto.ProblemDTO.Status;
import pl.dolecinski.subdicium.server.datastore.api.exception.ConstraintsViolationException;

/**
 * Simple Request entity. Representes Requested posted by end user.
 * Contains information about user, resource type, status, etc.
...
//    @Column(length = 2000, nullable = true)
     ...
     * sets actual Date as creationDate
     * sets Status as a UNSENT
     * and updates lastModificationDate
     */
    public Problem() {
    ...
    /**
     * Gets date of creation. Automatically initialized when this entity was created
     ...
     * @throws ConstraintsViolationException when trying to set status back to UNSENT
     */
    public void setStatus(Status status) {
        throwIfNull(status);
        if (this.status != Status.UNSENT && status == Status.UNSENT) {
            throw new ConstraintsViolationException("Cannot change status of request back to UNSENT");
        }
     ...
     * @param resourceName resource name, cannot be null
     */
     ...
     * @param userName, username, cannto be null
     */
     ...
     * sent.
     * @param authorMessage message to set
     */
    public void setAuthorMessage(String authorMessage) {
        if(this.status!=Status.UNSENT){
            throw new ConstraintsViolationException("Authors message can't be modified after sending request");
        }
    ...
    public String toString() {
        return "pl.edu.amu.wmi.kino.rrr.server.datastore.entities.Request[id=" + getId() + "]";
    }
