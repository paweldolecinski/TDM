...
class AppEngineConnectionRepository implements ConnectionRepository {
	private static final Logger log = Logger
			.getLogger(AppEngineConnectionRepository.class.getName());
	...
	public AppEngineConnectionRepository(String userId,
	...
	public MultiValueMap<String, Connection<?>> findAllConnections() {
		...
		Query query = new Query(getKind()).setAncestor(userKey)
				.addSort("providerId").addSort("rank");
	...
	public List<Connection<?>> findConnections(String providerId) {
		Query query = new Query(getKind()).setAncestor(userKey)
				...
				.addSort("rank");
		return DatastoreUtils.queryForList(datastore.prepare(query),
				connectionMapper);
	}
	...
	public MultiValueMap<String, Connection<?>> findConnectionsToUsers(
		...
		final Set<Entity> queryResultSet = new TreeSet<Entity>(
				new Comparator<Entity>() {
					@Override
					public int compare(Entity e1, Entity e2) {
						String providerId1 = (String) e1
								.getProperty("providerId");
						String providerId2 = (String) e2
								.getProperty("providerId");
						int compareToResult = providerId1
								.compareTo(providerId2);
		...
		for (Entry<String, List<String>> entry : providerUserIds.entrySet()) {
			...
			final Query query = new Query(getKind()).setAncestor(userKey)
					.setFilter(filter).addSort("providerId").addSort("rank");
	...
	public Connection<?> getConnection(ConnectionKey connectionKey) {
		final CompositeFilter filter = CompositeFilterOperator.and(
				FilterOperator.EQUAL.of("providerId",
		...
		try {
			...
			log.warning("Too many results were found for query "
					+ query.toString());
	...
	public void addConnection(Connection<?> connection) {
		...
		long rank = 1;
		// Find max rank -> Query sorting by rank in desc order and limiting
		// resultset to single entity
		...
		List<Entity> resultList = datastore.prepare(query).asList(
				FetchOptions.Builder.withLimit(1));
		Entity singleEntity = (resultList != null && resultList.size() > 0) ? resultList
				.get(0) : null;
		if (singleEntity != null)
			rank += (Long) singleEntity.getProperty("rank");

		String connectionKeyName = createConnectionKeyName(userId,
				connection.getKey());
		...
		final Key key = new KeyFactory.Builder(userKey).addChild(getKind(),
				connectionKeyName).getKey();
	...
	public void updateConnection(Connection<?> connection) {
		...
		String connectionKeyName = createConnectionKeyName(userId,
				connection.getKey());
		final Key connectionKey = new KeyFactory.Builder(userKey).addChild(
				getKind(), connectionKeyName).getKey();
		Transaction txn = datastore.beginTransaction();
		...
		try {
			...
			log.warning("There was the problem updating connection "
	...
	public void removeConnections(String providerId) {
		...
		for (ConnectionInterceptor interceptor : interceptingConnectionsTo(connections
				.iterator().next())) {
	...
	public void removeConnection(ConnectionKey connectionKey) {
		...
		try {
			...
			for (ConnectionInterceptor interceptor : interceptingConnectionsTo(connection)) {
				interceptor.beforeRemove(userId,
						Collections.singletonList(connection));
			}
			datastore.delete(txn, entity.getKey());
			txn.commit();
			for (ConnectionInterceptor interceptor : interceptingConnectionsTo(connection)) {
				interceptor.afterRemove(userId,
						Collections.singletonList(connection));
			}
		} catch (EntityNotFoundException e) {
			log.warning("There was problem deleted connection "
					+ connectionKey.toString() + ". No such connection exists.");
		} finally {
			if (txn.isActive())
				txn.rollback();
		}
	...
	private final class ServiceProviderConnectionMapper implements
			EntityMapper<Connection<?>> {
	...
	public static String createConnectionKeyName(String userId,
			ConnectionKey connectionKey) {
		return userId + "-" + connectionKey.getProviderId() + "-"
				+ connectionKey.getProviderUserId();
	}
	...
	public void addInterceptor(ConnectionInterceptor<?> interceptor) {
		Class<?> serviceApiType = GenericTypeResolver.resolveTypeArgument(
				interceptor.getClass(), ConnectionInterceptor.class);
		interceptors.add(serviceApiType, interceptor);
	}

	private List<ConnectionInterceptor<?>> interceptingConnectionsTo(
			Connection<?> connection) {
		Class<?> serviceType = GenericTypeResolver.resolveTypeArgument(
				connection.getClass(), Connection.class);
		List<ConnectionInterceptor<?>> typedInterceptors = interceptors
				.get(serviceType);
